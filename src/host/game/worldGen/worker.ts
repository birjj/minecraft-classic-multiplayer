/**
 * @fileoverview Yoinked from https://classic.minecraft.net/assets/js/RandomLevelWorker.js
 */
...
function PerlinNoise(random, levels = 8) {
    ...
    this.createLevel = function (seed, xSize, zSize, ySize) {
        ...
        this.grow = function (aint) {
            ...
            for (var l = 0; l < i; ++l) {
                ...
                for (var i1 = 0; i1 < j; ++i1) {
                    ...
                    var j1;
                    var k1 = Math.trunc(
                        ((j1 = Math.trunc(aint[l + i1 * i])) * this.zSize +
                            i1) *
                            this.xSize +
                            l
                    );
                        ...
                        if (
                            ...
                            if (flag) {
                                ...
                                if (
                                    ...
                                    for (i3 = 0; i3 < k2; ++i3) {
                                        this.tiles[
                                            l2 + i3 * this.xSize * this.zSize
                                        ] = 13; //(byte) Tile.treeTrunk.id;
                                    }
                                }
                            }
                        }
        ...
        this.placeOre = function (tile, j, k, l) {
            ...
            for (var l1 = 0; l1 < k1; ++l1) {
                ...
                var i2 = Math.trunc(
                    ((random.nextFloat() + random.nextFloat()) * 75.0 * j) /
                        100.0
                );
                ...
                for (var j2 = 0; j2 < i2; ++j2) {
                    ...
                    f3 += f4 * 0.2;
                    f4 =
                        (f4 *= 0.9) + (random.nextFloat() - random.nextFloat());
                    f5 = (f5 + f6 * 0.5) * 0.5;
                    f6 =
                        (f6 *= 0.9) + (random.nextFloat() - random.nextFloat());
                    var f7 =
                        (Math.sin((j2 * 3.141592653589793) / i2) * j) / 100.0 +
                        1.0;

                    for (
                        var k2 = Math.round(f - f7);
                        k2 <= Math.round(f + f7);
                        ++k2
                    ) {
                        for (
        ...
        this.floodFill = function (xc, yc, zc, unused, tile) {
            ...
            while (count > 0) {
                ...
                for (i3 = i3; i3 < j3; ++i3) {
                    ...
                    if (z > 0) {
                        if (
                            (flag3 = this.tiles[val - this.xSize] == 0) &&
                            !flag
                        ) {
        ...
        for (i1 = 0; i1 < l; ++i1) {
            ...
            for (var l3 = 0; l3 < i3; ++l3) {
                ...
                if (random.nextFloat() >= 0.3) {
                    ...
                    var f11 =
                        Math.sin((l3 * 3.141592653589793) / i3) * 2.5 + 1.0;

                    for (
                        var i4 = Math.trunc(f8 - f11);
                        i4 <= Math.trunc(f8 + f11);
                        ++i4
                    ) {
                        for (
                            var j4 = Math.trunc(f9 - f11);
                            j4 <= Math.trunc(f9 + f11);
                            ++j4
                        ) {
...
export function generateLevel(obj: { worldSize: number; seed: number }) {
    ...
    var height = 64;
    return level.createLevel(obj.seed, width, depth, height);
}
